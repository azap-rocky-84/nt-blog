    ...
    <div className={`w-full shadow-[rgba(7,_65,_210,_0.1)_0px_9px_30px] rounded-lg p-4 ${className}`}>
      ...
      {posts.map((item) => (
            <div key={item._id} className='flex space-x-3 flex-nowrap items-center'>
                <img className='aspect-square object-cover rounded-lg w-1/5' src={item?.image ? stables.UPLOAD_FOLDER_BASE_URL + item?.image : images.noImagePost} alt={item.title}/>
      ...
      {tags.length === 0 ? (
        ...
        {tags.map((item, index) => (
            <Link key={index} to="/" className='inline-block rounded-md px-3 py-1.5 bg-primary font-roboto text-xs text-white md:text-sm'>{item}</Link>
        ))}
      </div>
      )}
    </div>
