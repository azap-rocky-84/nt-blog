    ...
    <div className='flex flex-nowrap items-start gap-x-3 bg-[#F2F4F5] p-3 rounded-lg'>
        ...
        <div className='flex-1 flex flex-col'>
            ...
            <span className='text-xs text-dark-light'>{new Date(comment.createdAt).toLocaleDateString("en-US", {
                        ...
                        {!isEditing && (
                            <p className='font-opensans mt-[10px] text-colortext'>{comment.desc}</p>
                        )}
                        {isEditing && (
                            <CommentForm btnLabel="Modifica" formSubmitHandler={(value) => updateComment(value, comment._id)} formCancelHandler={() => setAffectedComment(null)} initialText={comment.desc}/>
                        )}
                        <div className='flex items-center gap-x-3 text-colortext font-roboto text-sm mt-3 mb-3'>
                        {isUserLoggined && (
                    ...
                    {isReplying && (<CommentForm btnLabel="Rispondi" formSubmitHandler={(value) => addComment(value, repliedCommentId, replyOnUserId)}
